...
const schema=z.object({name:z.string().trim().min(2),phone:z.string().trim().regex(/^[0-9()+\-\s]{8,20}$/, "Telefone inválido"),position:z.string().trim().min(2),hiredAt:z.coerce.date(),active:z.enum(["true","false"]).default("true")});
