...
export async function AppShell({ children }: { children: React.ReactNode }) { const session = await auth(); const visible = links.filter(([href]) => session?.user.role === "ADMIN" || !ADMIN_ONLY_ROUTES.has(href)); return <div className="shell"><aside className="sidebar"><div className="brand"><span className="brand-mark">B</span>Barracar</div><nav className="nav">{visible.map(([href,label])=><Link href={href} key={href}>{label}</Link>)}</nav><form action={logoutAction}><button type="submit">Sair</button></form></aside><main className="content"><header className="top"><div><strong>Gestão operacional</strong><div className="muted">{session?.user.name} · {session?.user.role}</div></div></header>{children}</main></div> }
